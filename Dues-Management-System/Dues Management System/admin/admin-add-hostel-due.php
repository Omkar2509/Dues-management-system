	...
	if(isset($_POST['submit'])){
		...
		if(mysqli_num_rows($student_query)!=1){
			...
			if(mysqli_num_rows($query)==0){
				...
				$query = mysqli_query($connection,"update hostel_due set due_amount=due_amount+'$amount', added_by='".$_SESSION['name']."', added_on=NOW(), reason='$reason' where roll_number='$roll'",$connection);
				if($query){
					header('location:admin-profile-hostel.php?error=none');
				}
				else{
