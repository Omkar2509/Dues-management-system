<?php
	...
	$connection = mysqli_connect("localhost", "root", "omi@7447", "dbms2");
	if(!$connection){
		die("Connection Failed");
	}
	else{
		//echo '<script>alert("Connection Established!"); </script>';
		mysqli_select_db($connection,'dbms2');
	}
	if(session_id()==""){
		session_start();
	}
...
//	    return $decrypted_string;
	...
	function confirmMail($email){
		...
		{
		  $message = new Message();
		  $message->setSender("omkarsarafnoduesproject@diems-nodues.iam.gserviceaccount.com");
		  $message->addTo($email);
	...
	function sendMail($email,$url){
		...
		{
		  $message = new Message();
		  $message->setSender("sarafomkar18@gmail.com");
		  $message->addTo($email);
