...
<html lang="en">
  ...
  <body>
	...
	<p style="text-align:center;"><img src="../img/images.jpg" class="img-fluid" alt="DIEMS"></p>
			...
			<?php
				if(isset($_GET['roll'])){
					...
					if(strcmp($r,"")==0&&strcmp($m,"")==0){
						$query1=mysqli_query($connection,"select * from hostel_due order by roll_number"); 
					}
					else if(strcmp($r,"")!=0&&strcmp($m,"")!=0){
						$query1=mysqli_query($connection,"select * from hostel_due where roll_number='$r' and added_by='$m' order by added_on");	
					}
					else if(strcmp($m,"")!=0){
						$query1=mysqli_query($connection,"select * from hostel_due where added_by='$m' order by roll_number");
					}
					else{
						$query1 = mysqli_query($connection,"select * from hostel_due where roll_number='$r' order by added_on");
					}
				}
				else{
					$query1=mysqli_query($connection,"select * from hostel_due order by roll_number");	
				...
				while($row = mysqli_fetch_assoc($query1)){
					echo '
						<tr>
							...
							<td>'.$row['added_by'].'</td>
							<td>'.$row['reason'].'</td>
							<td><a class="btn btn-primary" href="admin-remove-hostel-due.php?roll='.$row['roll_number'].'"><span class="glyphicon glyphicon-trash" title="Remove" aria-hidden="true"></span></a></td>
