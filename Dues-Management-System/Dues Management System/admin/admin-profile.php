...
<html lang="en">
  ...
  <body>
	  ...
	  <div class="container">
	    ...
	    <h3 class="navbar-text"><?php echo "Welcome ".$_SESSION['name'];echo " in DIEMS No-Dues Portal";?></h3>
	...
	<p style="text-align:center;"><img src="../img/images.jpg" class="img-fluid" alt="DIEMS"></p>
				...
				while($row = mysqli_fetch_assoc($query1)){
					echo '
						<tr>
							...
							<td>'.$row['added_by'].'</td>
							<td>'.$row['reason'].'</td>
							<td><a class="btn btn-primary" href="admin-remove-mess-due.php?roll='.$row['roll_number'].'"><span class="glyphicon glyphicon-trash" title="Remove" aria-hidden="true"></span></a></td>
		...
		<?php
			...
			if(isset($_GET['error'])){
				...
				else if(strcmp($msg,"Delete successful.")==0){
					echo "<script>
						document.getElementById('error-div').className = 'alert alert-success';
						document.getElementById('error-div').style.display = 'block';
						document.getElementById('error-glyphicon').className = 'glyphicon glyphicon-ok';
