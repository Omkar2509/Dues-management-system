<?php
	...
	if(session_id()==""){
  	...
  	if(isset($_POST['submit'])){
  		...
  		$query1 = mysqli_query($connection,"select * from admin_email where email='$email'");
  		if(mysqli_num_rows($query1)==0){
  			header('location:forgot-password.php?error=email');
  		}
  		else{
  			$keys = generateRandomString(10);
  			$query = mysqli_query($connection,"update `admin_email` set `key`='$keys' where `email`='$email';");
  			if($query){
				//Send mail....
